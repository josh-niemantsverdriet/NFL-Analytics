...
test('central projection is distinct from the 34-10 exact mode', () => {
  ...
  assert.equal(view.heading, 'Projected score');
  assert.equal(view.alternativesHeading, 'Most likely exact scores');
  assert.equal(view.alternatives[0], mode);
  assert.match(view.explanation, /minimizes expected point error/);
